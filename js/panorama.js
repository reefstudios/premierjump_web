...
(function($){
	function Panorama(el, options) {
		...
		this.defaults = {
			...
			titleAnimationStepLeft: 15,
			titleAnimationStepRight: 15,
						...
						if(parent.opts.eventsSettings.lastXYById[pointerId]!==undefined){
							...
							if(mNewY>waitY && mNewX<waitX){
								...
								if(parent.opts.busy==false){
									newY+=parent.getItem(parent.opts.currentIndex).find(".contents").scrollTop();
									curY+=parent.getItem(parent.opts.currentIndex).find(".contents").scrollTop();
								}
								parent.opts.busy=true;
								//parent.getItem(parent.opts.currentIndex).find(".contents_list").hide();
								parent.getItem(parent.opts.currentIndex).find(".contents").scrollTop(newY);
		...
		this.changeItem = function(rightDirection){
			...
			this.opts.busy = true;
			var titleAnimStep = this.opts.titleAnimationStepLeft;
			if(rightDirection){
				titleAnimStep = this.opts.titleAnimationStepLeft;
			} else {
				titleAnimStep = this.opts.titleAnimationStepRight;
			}
			if(this.opts.debug){
				if(rightDirection){
			...
			this.opts.titleSelector && this.getTitle().animate({
				marginLeft: (-leftItemIndex * titleAnimStep) + "%"
	...
	Panorama.prototype = {
		...
		prepare: function(controller){
			...
			var myHeight = $("#index").height();
			var unmargin = this.getItemsContainer().position().top;
			var itemsHeight = myHeight - unmargin-this.opts.bottom_margin;
			...
			this.getItems().each(function (index, item) {
				...
				//item_contents.width($(item).width());
			...
			this.getItemsContainer().fadeIn(function(){
				...
				_this.getItems().each(function () {
					$(this).css('height', '-='+unmargin+'px')
					   ...
					   item_contents = $(this).find('.contents');
						if($(this).find('.header').length){
							contents_height = contents_height-$(this).find('.header').height();
						}
						item_contents.height(contents_height);
				});
			});
		}
	};
