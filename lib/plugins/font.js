	...
	draw: function( text, x, y, align, color ) {
		...
		if(this.multiline){
			...
			for (i = 0 ; i < text_arr.length ; i++){
				temp_text = temp_text + " "+ text_arr[i];
				if(this.getWidth(temp_text+text_arr[i+1]) > this.widthForMultiline){
					ctx.fillText( temp_text || this.text,
								ig.system.getDrawPos( ( x || this.pos.x ) ), //- ig.game.screen.x ),
								ig.system.getDrawPos( ( y+(line_count*this.getSize()) || this.pos.y ) ) ); //- ig.game.screen.y ) );
					temp_text = "";
					line_count++;
				}
			}
